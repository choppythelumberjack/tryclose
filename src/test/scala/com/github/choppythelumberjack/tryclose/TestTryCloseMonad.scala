...
class TestTryCloseMonad extends TryCloseMonadSpec {
  ...
  "Using Map" - {
    ...
    "should close both layers if second fails to close" in {
      val output =
        TryClose(DummyCloseable("Outer", NotThrows), closeHandler("Outer"))
          .mapWithHandler(firstClosing => DummyCloseable("Inner", Throws), closeHandler("Inner"))
    ...
    "should not process to second statement if first throws exception" in {
      val output =
        TryClose(somethingThatThrowsException("Outer"), closeHandler("Outer"))
          .mapWithHandler(firstClosing => DummyCloseable("Inner", NotThrows), closeHandler("Inner"))
  ...
  "Using FlatMap" - {
    "should close both outer layers if two outer layers fail" in {
      val output =
        TryClose(DummyCloseable("Outer", Throws), closeHandler("Outer"))
          .flatMap(firstClosing => TryClose(DummyCloseable("Inner", Throws), closeHandler("Inner")))
          .flatMap(secondClosing => TryClose(DummyCloseable("Core", NotThrows)))
    ...
    "should close both layers if first fails to close" in {
      val output =
        TryClose(DummyCloseable("Outer", Throws), closeHandler("Outer"))
          .flatMap(firstClosing => TryClose(DummyCloseable("Inner", NotThrows), closeHandler("Inner")))
    ...
    "process to second statement if second throws exception" in {
      val output =
        TryClose(DummyCloseable("Outer", NotThrows), closeHandler("Outer"))
          .flatMap(firstClosing => TryClose(somethingThatThrowsException("Inner"), closeHandler("Inner")))
  ...
  "Using For Comprehension" - {
    "should close both outer layers if two outer layers fail" in {
      val output = for {
        firstClosing  <- TryClose(DummyCloseable("Outer", Throws), closeHandler("Outer"))
        secondClosing <- TryClose(DummyCloseable("Inner", Throws), closeHandler("Inner"))
    ...
    "should close both layers if first fails to close" in {
      val output = for {
        firstClosing <- TryClose(DummyCloseable("Outer", Throws), closeHandler("Outer"))
        ye <- TryClose(DummyCloseable("Inner", NotThrows), closeHandler("Inner"))
      } yield (ye)
    ...
    "should close both layers if second fails to close" in {
      val output = for {
        ...
        ye <- TryClose(DummyCloseable("Inner", Throws), closeHandler("Inner"))
      } yield (ye)
    ...
    "should not process to second statement if first throws exception" in {
      val output = for {
        firstClosing <- TryClose(somethingThatThrowsException("Outer"), closeHandler("Outer"))
        ye <- TryClose(DummyCloseable("Inner", NotThrows), closeHandler("Inner"))
      } yield (ye)
    ...
    "process to second statement if second throws exception" in {
      val output = for {
        firstClosing <- TryClose(DummyCloseable("Outer", NotThrows), closeHandler("Outer"))
        ye <- TryClose(somethingThatThrowsException("Inner"), closeHandler("Inner"))
      } yield (ye)
  ...
  "When transformed to self" - {
    ...
    "with flatMap - can only process transformation to self twice since can't do static analysis with monads" in {
      ...
      val output =
        TryClose(DummyCloseable("Outer", NotThrows), closeHandler("Outer"))
          .flatMap(firstClosing => TryClose(firstClosing, closeHandler("Inner")))
  ...
  "Using For Comprehensions - Try Monad should Handle Two Layers of Closeables and Exception" - {
    ...
    "Where both inner and outer layer closes fail" in {
      ...
      val output = for {
        ...
        res <- TryClose(somethingThatThrowsException("Throwing"), closeHandler("Core")) //, wrapCloseFailure = Throws
      } yield (res)
    ...
    "Where only inner layer fails" in {
      ...
      val output = for {
        ...
        mc2 <- TryClose(DummyCloseable("Inner", Throws), closeHandler("Inner"))
        res <- TryClose(somethingThatThrowsException("Throwing"), closeHandler("Core")) //, wrapCloseFailure = true
      } yield (res)
